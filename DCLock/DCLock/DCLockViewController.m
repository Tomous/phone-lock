...
@property (weak, nonatomic) IBOutlet DCLockIndicator *indecator;//九点指示图
@property (weak, nonatomic) IBOutlet DCLockView *lockview;//触摸田子控件
@property (weak, nonatomic) IBOutlet UILabel *tipLable;//输入密码
@property (weak, nonatomic) IBOutlet UIButton *tipButton;//重新输入

@property(assign,nonatomic)int nRetryTimesRemain; // 剩余几次输入机会
...
@implementation DCLockViewController
...
- (id)initWithType:(DCLockViewType)type
{
    self = [super init];
    if (self) {
        _nLockViewType = type;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    switch (_nLockViewType) {
        case DCLockViewTypeCheck:
        {
            _tipLable.text = @"请输入解锁密码";
        }
            break;
        case DCLockViewTypeCreate:
        {
            _tipLable.text = @"创建手势密码";
        }
            break;
        case DCLockViewTypeModify:
        {
            _tipLable.text = @"请输入原来的密码";
        }
            break;
        case DCLockViewTypeClean:
        default:
        {
    ...
    // 尝试机会
    self.nRetryTimesRemain = DCLockRetryTimes;
    ...
    // 本地保存的手势密码
    self.savedPassword = [DCLockPassword loadLockPassword];
...
- (void)checkPassword:(NSString*)string
{
    ...
    if ([string isEqualToString:self.savedPassword]) {
        ...
        if (_nLockViewType == DCLockViewTypeModify) { // 验证旧密码
            
            self.passwordOld = string; // 设置旧密码，说明是在修改
            
            [self setTip:@"请输入新的密码"]; // 这里和下面的delegate不一致，有空重构
            
        } else if (_nLockViewType == DCLockViewTypeClean) { // 清除密码
            
            [DCLockPassword saveLockPassword:nil];
            [self hide];
    ...
    // 验证密码错误
        ...
        if (self.nRetryTimesRemain > 0) {
            ...
            if (1 == self.nRetryTimesRemain) {
                ...
                [self setErrorTip:[NSString stringWithFormat:@"密码错误，还可以再输入%d次", self.nRetryTimesRemain]
                        errorPswd:string];
            }
...
- (void)createPassword:(NSString*)string
{
    ...
    // 确认输入密码
        ...
        if ([self.passwordNew isEqualToString:self.passwordconfirm]) {
            ...
            [DCLockPassword saveLockPassword:string];
...
{
    NSLog(@"重设TipButton");
    if ((_nLockViewType == DCLockViewTypeCreate || _nLockViewType == DCLockViewTypeModify) &&
...
- (void)hide
{
    switch (_nLockViewType) {
        ...
        case DCLockViewTypeModify:
        {
            [DCLockPassword saveLockPassword:self.passwordNew];
        }
            break;
        case DCLockViewTypeClean:
        default:
